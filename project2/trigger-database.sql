...
drop table if exists ffairlines;
...
create table ffairlines (customerid char(10) references newcustomers ON DELETE CASCADE, airlineid char(2) references airlines(airlineid), points int, status char(10));
insert into ffairlines (with temp as (select customerid, 
                            airlineid, 
                            SUM(TRUNC(EXTRACT(EPOCH FROM local_arrival_time - local_departing_time)/60)) as points,
                            CASE WHEN SUM(TRUNC(EXTRACT(EPOCH FROM local_arrival_time - local_departing_time)/60)) >= 750 THEN 'GOLD'
                                 WHEN SUM(TRUNC(EXTRACT(EPOCH FROM local_arrival_time - local_departing_time)/60)) >= 500 THEN 'SILVER'
