...
cur.execute("WITH temp_points as (select coalesce(sum(trunc(extract(epoch from local_arrival_time - local_departing_time)/60)), 0) as points from flewon natural join flights where customerid = 'cust4' AND airlineid = 'SW'), " \
            "temp_status as (select CASE WHEN points >= 750 THEN 'GOLD' " \
            "WHEN points >= 500 THEN 'SILVER' " \
            "ELSE 'BRONZE' END as status from temp_points) " \
            "UPDATE ffairlines set airlineid = 'SW', points = (select * from temp_points), status = (select * from temp_status) where customerid = 'cust4' and airlineid = 'AA'")
conn.commit()

printAll('After updating AA to SW as ff airline.  SW should now be the frequentflieron for cust4')

cur.execute("with temp_points as (select coalesce(sum(trunc(extract(epoch from local_arrival_time - local_departing_time)/60)), 0) as points from flewon natural join flights where customerid = 'cust4' AND airlineid = 'AA'), " \
            "temp_status as (select CASE WHEN points >= 750 THEN 'GOLD' " \
            "WHEN points >= 500 THEN 'SILVER' " \
            "ELSE 'BRONZE' END as status from temp_points) " \
            "INSERT into ffairlines VALUES ('cust4', 'AA', (select * from temp_points), (select * from temp_status));")
conn.commit()
...
cur.execute("with temp_points as (select coalesce(sum(trunc(extract(epoch from local_arrival_time - local_departing_time)/60)), 0) as points " \
"from flewon natural join flights where customerid = 'cust4' AND airlineid = 'UA')," \
"temp_status as (select CASE WHEN points >= 750 THEN 'GOLD' " \
"WHEN points >= 500 THEN 'SILVER' " \
"ELSE 'BRONZE' END as status from temp_points)" \
"insert into ffairlines VALUES ('cust4', 'UA', (select * from temp_points), (select * from temp_status));")
cur.execute("with temp_points as (select coalesce(sum(trunc(extract(epoch from local_arrival_time - local_departing_time)/60)), 0) as points " \
"from flewon natural join flights where customerid = 'cust4' AND airlineid = 'AA')," \
"temp_status as (select CASE WHEN points >= 750 THEN 'GOLD' " \
"WHEN points >= 500 THEN 'SILVER' " \
"ELSE 'BRONZE' END as status from temp_points)" \
"insert into ffairlines VALUES ('cust4', 'AA', (select * from temp_points), (select * from temp_status));")
